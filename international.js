...
//Grab Json news Data
window.addEventListener('online', () => intNews(sourceSelector.value));

window.addEventListener('load', e => {
  sourceSelector.addEventListener('change', evt => intNews(evt.target.value));
  grabNewsSources().then(() => {
    sourceSelector.value = defaultSource;
    intNews();
  });
...
async function intNews(source = defaultSource) {
